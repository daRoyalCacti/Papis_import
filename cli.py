...
_DEBUG_TSV_HEADER = [
    ...
    "Candidate Sources", "Title", "Authors", "Year", "DOI", "ISBN", "arXiv",
    "Notes", "Imported", "Error", "Suggested Command",
]
...
_PROFILE_TSV_HEADER = [
    ...
    "Text LLM s", "Vision LLM s", "Vision Pacing s", "Identifier Lookups s",
...
def _title_search_timings_json(timing: TimingBreakdown) -> str:
    return json.dumps(
        [asdict(item) for item in timing.title_searches],
        ensure_ascii=True,
        sort_keys=True,
        separators=(",", ":"),
    )
